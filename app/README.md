...
> **D1 (resolved):** this is the existing mockup *productionized in place* — the
> `<x-dc>` template, design tokens, SVG geometry, copy, and choreography are
> frozen; split 07 only swapped the dummy data for live `/api` calls.
...
client + formatters on `window.FR`.
...
Then open `http://localhost:5500/`. Point the page at a running API one of:
...
- the default same-origin `/api` (split 13 reverse-proxies this in production).
...
states against §06-shaped mock responses; its browser binary is environment-gated
